<#
.SYNOPSIS
    External data source for Tools.

.DESCRIPTION
    This file contains curated bookmark data for Tools.
    It is loaded by bookmarks.ps1 as an external data source and merged into the
    main curated bookmarks structure.

.NOTES
    - This file must be saved with UTF-8 with BOM encoding for PowerShell to properly parse it
    - The file returns a hashtable with 'Europe' as the key
    - Any Cyrillic or special characters in titles should use Latin transliterations
      to avoid encoding issues
    - URLs can contain special characters as they are in single-quoted strings
#>
